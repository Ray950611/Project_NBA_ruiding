...
def similarity_cos(a,b,w):
    ...
    return similarity
def nn(player_stat,base,basenames):
    weight = np.array([5,4,3,2,5,5,5,5,5,5,4,2,3,5,1,3,4,4,4,2,4,5,3,4,4,4,3,2,4])
    sim_list = []
    for i in range(len(base)):
        name = basenames[i]
        stat_other = base[i]

        diff_1 = 0.5*sum(weight*abs(player_stat-stat_other))/sum(weight)
        #diff_2 = 1-similarity_cos(player_stat,stat_other,weight)
        #diff = abs((diff_1+diff_2)*0.5)
        sim_list.append([round(diff_1,4),name])
    nn = sorted(sim_list)
    return nn
...
@app.route('/season_methods/')
def season_methods():

   return render_template('season_methods.html')
...
def all_contracts():
    return render_template('salary_group.html')
@app.route("/contract_methods/")
def contract_methods():
    return render_template('contract_methods.html')
...
def player_contract():
    ...
    for i in stats.tbody.find_all('tr'):
        ...
        if float(mp) > 10.0 and float(gp)>10:
            ...
            if name not in adv_data.keys() and per>=0 and wS>=0:
                adv_data.update({name:[pos_num,age,mp,per,ts,att3,trb,asr,slr,blr,tov,usg,owS,dwS,vorp]})
    ...
    for i in stats.tbody.find_all('tr'):
        ...
        name = str(row[0])
        if name in adv_data.keys() and len(adv_data[name])==15:
            fg=0
            if len(row[9])!=0:
                fg = float(row[9])
            fa = float(row[8])
    ...
    result = int(cap_value*ratio)
        ...
        #result = int(cap_value*ratio)
    Result = "Position: "+Pos[pos]+" "+player_name+" : $"+str(result*10000)+"\n"
    ...
    for name in adv_data.keys():
        ...
        if len(adv_data[name])==29:
            #pos = adv_data[name][0]
            total_names.append(name)
            total_data.append(adv_data[name])
            #cul_data[pos].append(adv_data[name])
            #names_data[pos].append(name)
    Classifications = ['Pure Shooters','Rim Protectors','Star Scorers','Bench Role Players','Average Guards','Star Bigs','3-D','Floor Spacing Wings','Backup Bigs','Average Bigs']
    X =  preprocessing.scale(total_data)
    kmeans = KMeans(n_clusters=10).fit(X)
    p_index = int(total_names.index(player_name))
    player_stat = X[p_index]
    p_class = kmeans.labels_[p_index]
    player_role = Classifications[p_class]
    base = X[kmeans.labels_==p_class]
    basenames=np.array(total_names)[kmeans.labels_==p_class]
    #result+="<h1>Player Role: "+str(player_role)+"</h1>"
    NN = nn(player_stat,base,basenames)[1:4]
    #sim = NN[0][1]+','+NN[1][1]+','+NN[2][1]+','+NN[3][1]+','+NN[4][1]
    result+="<h1>Similar Players by Role:</h1>"
    for i in range(3):
    ...
    #for i in range(10):
        ...
        #result+="<h2>"+str(np.array(total_names)[kmeans.labels_==i])+"</h2>"
    return render_template('contract_result.html',image = Image,result=result)
@app.route("/game/")
def form_game():

   return render_template('form_game.html')
@app.route("/game_methods/")
def game_methods():

   return render_template('game_methods.html')
